...
//Not a good place for this if we break this out into library, but gotta have somewhere
type err struct {
	code int
	error
}
...
var (
	ErrEventInvalidEventType = err{
		code:  http.StatusBadRequest,
		error: errors.New("Event Type Required")}
	ErrEventInvalidCloudEventsVersion = err{
		code:  http.StatusBadRequest,
		error: errors.New("Cloud Event Version Required")}
	ErrEventInvalidSource = err{
		code:  http.StatusBadRequest,
		error: errors.New("Source Required")}
	ErrEventInvalidEventID = err{
		code:  http.StatusBadRequest,
		error: errors.New("Event ID Required")}
	ErrEventInvalidEventTime = err{
		code:  http.StatusBadRequest,
		error: errors.New("Event Time Required")}
	ErrEventInvalidSourceURL = err{
		code:  http.StatusBadRequest,
		error: errors.New("Source URL Required")}
	ErrEventInvalidContentType = err{
		code:  http.StatusBadRequest,
...
func (e *Event) Validate() error {
	if e.EventType == "" {
		return ErrEventInvalidEventType
	}
	if e.CloudEventsVersion == "" {
		return ErrEventInvalidCloudEventsVersion
	}
	if e.Source == "" {
		return ErrEventInvalidSource
	}
	if e.EventID == "" {
		return ErrEventInvalidEventID
	}
	if e.EventTime == common.NewDateTime() {
		return ErrEventInvalidEventTime
	}
	if e.SchemaURL == "" {
		return ErrEventInvalidSourceURL
	}
	if e.ContentType == "" {
		return ErrEventInvalidContentType
	}
	return nil
}
