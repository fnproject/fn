...
func runAndAssert(t *testing.T, s *fnTest.SuiteSetup, fnRoute, fnImage,
	fnFormat string, requestPayload interface{}, responsePayload interface{}) (*bytes.Buffer, http.Header) {
...
func TestFDKFormatSmallBody(t *testing.T) {
	...
	for _, format := range formats {
		...
		// response:
		//    "Hello John"
		t.Run(fmt.Sprintf("test-fdk-hello-john-%v-small-body", format), func(t *testing.T) {
			...
			output, headers := runAndAssert(t, s, route, FDKImage, format, helloJohnPayload, responsePayload)
